		...
		<div class="container-fluid">
			...
			<div class="container-fluid">
				...
				<div class="col-md-10">
					...
					<%-- 显示热门商品 --%>
					<c:choose>
						<c:when test="${!empty hotProductList}">
							<c:forEach var="hotProduct" items="${hotProductList}">
								<div class="col-md-2" style="text-align:center;height:200px;padding:10px 0px;">
									...
									<p><a href="${pageContext.request.contextPath}/product?method=findProductByPid&pid=${product.pid}" style='color:#666'>${hotProduct.pname}</a></p>
									<p><font color="#E4393C" style="font-size:16px">&yen;${hotProduct.shop_price}</font></p>
								</div>
							</c:forEach>
						</c:when>
						<c:otherwise>
							无热门商品，请访问default.jsp页面
						</c:otherwise>
					</c:choose>
					...
					<c:choose>
						<c:when test="${!empty newProductList}">
							<c:forEach var="newProduct" items="${newProductList}">
								<div class="col-md-2" style="text-align:center;height:200px;padding:10px 0px;">
									<a href="/11_shopV1.0/product?method=findProductByPid&pid=1">
										<img src="${newProduct.pimage}" width="130" height="130" style="display: inline-block;">
									</a>
									<p><a href="/11_shopV1.0/product?method=findProductByPid&pid=1" style='color:#666'>${newProduct.pname}</a></p>
									<p><font color="#E4393C" style="font-size:16px">&yen;${newProduct.shop_price}</font></p>
								</div>
							</c:forEach>
						</c:when>
						<c:otherwise>
							无最新商品，请访问default.jsp页面
						</c:otherwise>
					</c:choose>
